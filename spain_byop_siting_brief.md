...
Marc wants high autonomy here, not a back-and-forth clarifying-questions loop, and wants this **one-shotted**: research, planning, and a working dashboard/model in a single autonomous run, not a stop-and-wait-for-review after the plan. This is the last session Fable will be usable for, so the instruction is to go all-in and try to surprise Marc with what gets built, not to play it safe and hand back a plan for approval.

1. **Research.** Actually go find out: what's downloadable vs. merely browsable on MITECO Geoportal, what the "oferta y demanda" layers contain, whether PVGIS/AEMET have usable solar and climate data for Spain, what catastro access actually looks like in practice, and what real primary material exists on regional/municipal political reception of datacenters (Aragon's fast-tracking, any other region's resistance). Resolve as many of the open questions in this doc as the evidence allows. Where evidence is genuinely inconclusive, say so plainly rather than guessing.
2. **High-level plan.** Based on what the research actually found (not on assumptions made before finding it), decide: a unit of analysis, a narrowed variable shortlist with reasoning for what got cut, an approach to the scoring/weighting mechanism, and a data pipeline sketch. Make the calls — don't punt these back as open questions.
3. **Build it.** Go straight from plan into a working version of the dashboard/model described in the aspirational end-state above (toggleable variables, live re-weighting, size-mode partitioning, click-for-detail) — as much of it as is achievable, aiming to overshoot rather than undershoot. Keep the code itself minimal and clean (Marc is budget-constrained on tokens, not on ambition) — favor a lean implementation over an elaborate one, even while being ambitious about what the *model and data* cover. Asking Marc for feedback mid-build is fine if something is genuinely a fork worth surfacing, but the default mode is to decide and keep moving, not to check in.
4. **Self-checkpointing.** Given session/context limits are a real constraint here, maintain a running progress log (what's been checked, what was found, what's been built, what's still open) and update it at natural breakpoints — end of each research question, end of each build milestone — so that if the session cuts off mid-task, the work up to that point is not lost and is resumable.
...
## A tangential but real second audience: land buyers, not just recruiters

Beyond the SemiAnalysis work-sample purpose, Marc wants this to be genuinely useful to someone trying to source and accumulate land in Spain for future datacenter development — a PE firm, an infrastructure fund, or an operator's land-acquisition team. His observation: right now there isn't much competition for large parcels of Spanish land suited to this use case, but that will likely change as the BYOP/datacenter buildout thesis becomes more widely recognized — meaning land itself, not just the datacenter on it, is an under-the-radar value-accumulation opportunity. Marc intends to take this product to actual engineers/operators at PE or private capital firms for feedback, so it needs to hold up as something a land-sourcing professional would find genuinely useful, not just a recruiting demo. Keep this in mind as an implicit second bar for "is this valuable" — it should help someone answer "where should I be trying to buy or option land right now, before anyone else is looking," not just "where's a good datacenter site" in the abstract.
